...
export function PainelAdmin({ onVoltar }) {
  ...
  const [aba,setAba]=useState("pendentes"); // pendentes | ativos | carrossel
    ...
    <div style={{minHeight:"100vh",background:C.bg,padding:"18px 16px 80px"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
        ...
        <div>
          ...
          <div style={{color:C.textDim,fontSize:11,fontFamily:"monospace"}}>Gestão · {usuarios.length} cadastrados</div>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16}}>
        <button onClick={()=>setAba("pendentes")} style={{flex:1,padding:"9px",borderRadius:9,cursor:"pointer",fontWeight:800,fontSize:12,
          border:`2px solid ${aba==="pendentes"?C.warningLight:C.border}`,
          background:aba==="pendentes"?C.warning+"33":C.tagBg,
          color:aba==="pendentes"?C.warningLight:C.textMuted}}>
        ...
        <button onClick={()=>setAba("ativos")} style={{flex:1,padding:"9px",borderRadius:9,cursor:"pointer",fontWeight:800,fontSize:12,
          border:`2px solid ${aba==="ativos"?C.accent:C.border}`,
          background:aba==="ativos"?C.accentDark:C.tagBg,
          color:aba==="ativos"?C.white:C.textMuted}}>
          Ativos ({ativos.length})
        </button>
        <button onClick={()=>setAba("carrossel")} style={{flex:1,padding:"9px",borderRadius:9,cursor:"pointer",fontWeight:800,fontSize:12,
          border:`2px solid ${aba==="carrossel"?C.blueLight:C.border}`,
          background:aba==="carrossel"?"rgba(26,92,204,0.2)":C.tagBg,
          color:aba==="carrossel"?C.blueLight:C.textMuted}}>
          🖼️ Carrossel
        </button>
      </div>

      {/* ── Aba Carrossel ── */}
      {aba==="carrossel" && <PainelCarrossel/>}
      ...
      {aba!=="carrossel"&&<button onClick={carregar} style={{width:"100%",marginTop:8,padding:11,borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:12,background:C.tagBg,border:`1px solid ${C.border}`,color:C.textMuted}}>↻ Atualizar lista</button>}
    </div>
  );
}
